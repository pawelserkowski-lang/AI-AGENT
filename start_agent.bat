...
REM  run_agent.py, which installs Python dependencies if needed, checks
REM  for Ollama and downloads the model before starting main.py. You can
REM  modify the values below to suit your needs. On Windows, setting the
REM  code page to UTF-8 ensures that Polish characters display correctly.
REM
REM  Usage:
...
REM after run_agent.py below. For example, to specify a different .env file
REM or log level, append arguments here.
%PYTHON_EXEC% run_agent.py
